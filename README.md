...
1. **Windows:** obtain a publicly trusted Authenticode code-signing identity. A self-signed certificate is useful for local testing only and is not suitable for a public release. The current workflow accepts an exportable, password-protected `.pfx`; before buying a certificate, confirm that the CA or signing service permits this delivery method. Modern code-signing certificates are commonly held in a hardware or cloud HSM and cannot be exported as PFX. Such a service requires a dedicated GitHub Actions integration instead of `WINDOWS_PFX_BASE64`.

   When an exportable CA-issued PFX is available, copy its bytes as base64 into `WINDOWS_PFX_BASE64` and add its export password as `WINDOWS_PFX_PASSWORD`:
...
2. **Tauri updater and Android:** on Windows, the repository can create the remaining seven secrets together. The helper uses cryptographically random passwords, creates an encrypted Tauri updater key and a standard PKCS#12 Android release keystore, restricts the output directory to the current Windows account and SYSTEM, and protects stored passwords with Windows DPAPI. It refuses to overwrite an existing signing directory.

   ```powershell
   npm run signing:generate
   ```

   The default output is `%USERPROFILE%\Documents\Notide Signing Keys`. It is deliberately outside the repository. OpenSSL is required; use the script's `-OpenSslPath` parameter when it is installed elsewhere.

   Copy each value without printing it, create a GitHub repository Secret with the same name, paste it, and then continue with the next name:
...
Keep an encrypted offline backup of the Windows identity and the complete `Notide Signing Keys` directory. DPAPI-protected passwords can be decrypted only by the Windows account that created them, so a copy of that directory alone is not a portable password backup. GitHub does not allow reading secret values back after saving them. Losing or replacing the Android or updater key prevents existing installations from accepting future updates.
